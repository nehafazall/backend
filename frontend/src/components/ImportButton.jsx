...
const ImportButton = ({ templateType, title, onSuccess }) => {
    ...
    const downloadTemplate = () => {
        ...
        const headers = template.headers.map(h => h.replace('*', '')).join(',');
        const exampleValues = template.headers.map(h => {
            const key = h.replace('*', '');
            return template.example_row[key] || '';
        }).join(',');
    ...
    const handleFileSelect = (e) => {
        ...
        reader.onload = (event) => {
            ...
            setParsedData(data);
            toast.success('File parsed: ' + data.length + ' rows found');
        };
    ...
    const handleImport = async () => {
        ...
        setImporting(true);
        try {
            const endpoint = '/import/' + (templateType === 'students_cs' ? 'students/cs' : 
                                           templateType === 'students_mentor' ? 'students/mentor' : 
                                           templateType);
            ...
            if (res.data.failed > 0) {
                toast.error(res.data.failed + ' records failed');
            }
        } catch (err) {
                        ...
                        <div className="space-y-4">
                            ...
                            <Card>
                                ...
                                <CardContent className="py-2">
                                    <ul className="text-sm space-y-1 list-disc list-inside text-muted-foreground">
                                        {template.instructions.map((inst, idx) => (
                                            <li key={idx}>{inst}</li>
                                        ))}
                                    </ul>
                                </CardContent>
                            ...
                            {parsedData && (
                                <div className="bg-muted/50 rounded-lg p-4">
                                    <p className="font-medium">Ready to import: {parsedData.length} records</p>
                                    <p className="text-sm text-muted-foreground mt-1">
                                        Preview: {parsedData.slice(0, 3).map(r => r.full_name || r.phone).join(', ')}
                                        {parsedData.length > 3 && '...'}
                                    </p>
                                </div>
                            )}
                        ...
                        <div className="space-y-4">
                            ...
                            {results.errors && results.errors.length > 0 && (
                                <Card>
                                    ...
                                    <CardContent className="py-2 max-h-40 overflow-y-auto">
