...
const MilesDepositsPage = () => {
    ...
    const fetchRecords = useCallback(async () => {
        setLoading(true);
        try {
            const response = await fetch(`${API_URL}/api/finance/miles/deposits`, {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const url = editingId 
                ? `${API_URL}/api/finance/miles/deposits/${editingId}`
                : `${API_URL}/api/finance/miles/deposits`;
    ...
    const handleDelete = async (id) => {
        ...
        try {
            const response = await fetch(`${API_URL}/api/finance/miles/deposits/${id}`, {
                method: 'DELETE',
                headers: { 'Authorization': `Bearer ${token}` }
            });
            if (response.ok) {
    ...
    const totalDeposits = records.reduce((sum, r) => sum + (r.amount_in_aed || 0), 0);

    return (
        <div className="space-y-6" data-testid="miles-deposits-page">
            <Card>
                <CardHeader>
                    <CardTitle className="text-blue-500">Miles Capitals - Deposits</CardTitle>
                    <CardDescription>Record capital deposits</CardDescription>
                </CardHeader>
                <CardContent>
                    <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                        ...
                        <Select value={form.payment_method} onValueChange={(v) => handleChange('payment_method', v)}>
                            <SelectTrigger data-testid="deposit-method">
                        ...
                        <Select value={form.currency} onValueChange={(v) => handleChange('currency', v)}>
                            <SelectTrigger data-testid="deposit-currency">
                                <SelectValue />
                            </SelectTrigger>
                            <SelectContent>
                        ...
                        <div className="lg:col-span-3 flex justify-end gap-2">
                            ...
                            <Button type="submit" disabled={saving} className="bg-blue-600 hover:bg-blue-700">
                                ...
                                {saving ? 'Saving...' : (editingId ? 'Update' : 'Add Deposit')}
                            </Button>
                        </div>
                    </form>
                </CardContent>
            ...
            <div className="grid gap-4 md:grid-cols-2">
                <Card>
                    <CardContent className="pt-6">
                        ...
                        <div className="text-3xl font-bold text-green-500">AED {formatNumber(totalDeposits)}</div>
                    </CardContent>
                </Card>
                <Card>
                    <CardContent className="pt-6">
...
export default MilesDepositsPage;
