...
const SettingsPage = () => {
    ...
    const download3CXTemplate = async () => {
        try {
            ...
            const fileName = `3cx_template_${data.extension}.json`;
    ...
    return (
        <div className="space-y-6 max-w-4xl mx-auto" data-testid="settings-page">
            ...
            <NotificationsCard 
                ...
                savingSound={savingSound}
            />

            <IntegrationsCard
                templateData={templateData}
                loading3CX={loading3CX}
                onFetchTemplate={fetch3CXTemplate}
                onDownload={download3CXTemplate}
...
const ProfileCard = ({ user, getRoleBadgeColor, formatRole, onEdit }) => (
    <Card>
        <CardHeader>
        ...
        <CardContent className="space-y-4">
            ...
            {user?.additional_phones?.length > 0 && (
                <div className="mt-2">
                    <span className="text-sm text-muted-foreground">Additional Numbers:</span>
                    <div className="flex flex-wrap gap-2 mt-1">
                        {user.additional_phones.map((phone, idx) => <Badge key={idx} variant="outline">{phone}</Badge>)}
                    </div>
                </div>
            )}
        </CardContent>
...
const ThemeCard = ({ theme, toggleTheme }) => (
    <Card>
        <CardHeader>
...
const NotificationsCard = ({ soundEnabled, onToggleSound, savingSound }) => (
    <Card>
        <CardHeader>
        ...
        <CardContent className="space-y-4">
            <div className="flex items-center justify-between">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <p className="text-sm text-muted-foreground">Play sound alerts for new notifications and leads</p>
                    </div>
                </div>
            ...
            <div className="space-y-2">
                ...
                <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2"><CheckCircle className="h-4 w-4 text-blue-500" /><span className="text-sm">New Lead Alert</span></div>
                    <Button variant="ghost" size="sm" disabled>Enabled</Button>
                </div>
                <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2"><CheckCircle className="h-4 w-4 text-green-500" /><span className="text-sm">System Notifications</span></div>
                    <Button variant="ghost" size="sm" disabled>Enabled</Button>
                </div>
            </div>
        </CardContent>
...
const IntegrationsCard = ({ templateData, loading3CX, onFetchTemplate, onDownload, onCopy }) => (
    <Card>
        <CardHeader>
            <div className="flex items-center gap-2"><Phone className="h-5 w-5" /><CardTitle>3CX Integration</CardTitle></div>
            <CardDescription>Connect your phone system for click-to-call and call journaling</CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
            <div className="flex items-center gap-2">
                ...
                <div className="space-y-3 p-4 bg-muted rounded-lg">
                    <div className="flex items-center justify-between">
                        ...
                        <div className="flex items-center gap-2">
                            ...
                            <Button variant="ghost" size="sm" onClick={() => onCopy(templateData.extension)}><Copy className="h-3 w-3" /></Button>
                        </div>
                    </div>
                    <div className="flex items-center justify-between">
                        <span className="text-sm font-medium">Webhook URL:</span>
                        <div className="flex items-center gap-2">
                            <code className="bg-background px-2 py-1 rounded text-xs max-w-xs truncate">{templateData.webhook_url}</code>
                            <Button variant="ghost" size="sm" onClick={() => onCopy(templateData.webhook_url)}><Copy className="h-3 w-3" /></Button>
                        </div>
                    </div>
